...
public class BCEnergyMenuTypes {
    public static final MenuType<ContainerEngineIron_BC8> ENGINE_IRON = IForgeMenuType.create((windowId, inv, data) -> {
        if (inv.player.getLevel().getBlockEntity(data.readBlockPos()) instanceof TileEngineIron_BC8 tile) {
    ...
    public static final MenuType<ContainerEngineStone_BC8> ENGINE_STONE = IForgeMenuType.create((windowId, inv, data) -> {
        if (inv.player.getLevel().getBlockEntity(data.readBlockPos()) instanceof TileEngineStone_BC8 tile) {
            MessageUtil.clientHandleUpdateTileMsgBeforeOpen(tile, data);
    ...
    public static final MenuType<ContainerEngineRF> ENGINE_RF = IForgeMenuType.create((windowId, inv, data) -> {
        if (inv.player.getLevel().getBlockEntity(data.readBlockPos()) instanceof TileEngineRF tile) {
            MessageUtil.clientHandleUpdateTileMsgBeforeOpen(tile, data);
            return new ContainerEngineRF(BCEnergyMenuTypes.ENGINE_RF, windowId, inv.player, tile);
        }
        return null;
    });
    public static final MenuType<ContainerDynamoMJ> DYNAMO_MJ = IForgeMenuType.create((windowId, inv, data) -> {
        if (inv.player.getLevel().getBlockEntity(data.readBlockPos()) instanceof TileDynamoMJ tile) {
            MessageUtil.clientHandleUpdateTileMsgBeforeOpen(tile, data);
            return new ContainerDynamoMJ(BCEnergyMenuTypes.DYNAMO_MJ, windowId, inv.player, tile);
        }
        return null;
    });
