...
public class GuiZonePlanner extends GuiBC8<ContainerZonePlanner> {
    ...
    private static final RenderType CUBOID_LINES_RENDER_TYPE;
    ...
    static {
        ...
        CUBOID_LINES_RENDER_TYPE = RenderType.create("buildcraft_zone_planner_gui_lines", DefaultVertexFormat.POSITION_COLOR, VertexFormat.Mode.LINE_STRIP, 256, false, true, rendertype$state);
    }
...
//        GLU.gluUnProject(screenX, screenY, 1f, modelViewBuffer, projectionBuffer, viewportBuffer, positionFarBuffer);
        Matrix4f mul = new Matrix4f(modelViewStack.last().pose());
        mul.multiply(projectionMatrix);
        unproject(mul, (float) screenX, (float) screenY, 0f, viewport, positionNear);
        unproject(mul, (float) screenX, (float) screenY, 1f, viewport, positionFar);

//        Vector3d rayStart = new Vector3d(positionNearBuffer.get(0), positionNearBuffer.get(1), positionNearBuffer.get(2));
        Vector3d rayStart = new Vector3d(positionNear.x(), positionNear.y(), positionNear.z());
        Vector3d rayPosition = new Vector3d(rayStart.x, rayStart.y, rayStart.z);
//        Vector3d rayDirection = new Vector3d(positionFarBuffer.get(0), positionFarBuffer.get(1), positionFarBuffer.get(2));
        Vector3d rayDirection = new Vector3d(positionFar.x(), positionFar.y(), positionFar.z());
        rayDirection.add(new Vector3d(-rayStart.x, -rayStart.y, -rayStart.z));
        normalize(rayDirection);

        for (int i = 0; i < 10000; i++) {
//            int chunkX = (int) Math.round(rayPosition.getX()) >> 4;
            int chunkX = (int) Math.round(rayPosition.x) >> 4;
//            int chunkZ = (int) Math.round(rayPosition.getZ()) >> 4;
            int chunkZ = (int) Math.round(rayPosition.z) >> 4;
...
//                        Math.round(rayPosition.getX()) - (chunkX << 4),
                        (int) Math.round(rayPosition.z) - (chunkX << 4),
//                        Math.round(rayPosition.getY()),
                        (int) Math.round(rayPosition.y),
//                        Math.round(rayPosition.getZ()) - (chunkZ << 4)
                        (int) Math.round(rayPosition.z) - (chunkZ << 4)
                );
...
//        GLU.gluPerspective(70.0F, (float) sizeX / sizeY, 1F, 10000.0F);
        Matrix4f projectionMatrix = Matrix4f.perspective(70.0F, (float) sizeX / sizeY, 1F, 10000.0F);
...
//        GlStateManager.rotate(90, 1, 0, 0); // look down
        modelViewStack.mulPose(Vector3f.XP.rotationDegrees(90.0F));
//        GlStateManager.pushMatrix();
        modelViewStack.pushPose();
...
//        GlStateManager.disableBlend();
    ...
    private static void normalize(Vector3d vector3d) {
        double invLength = Mth.fastInvSqrt(Math.fma(vector3d.x, vector3d.x, Math.fma(vector3d.y, vector3d.y, vector3d.z * vector3d.z)));
        vector3d.x = vector3d.x * invLength;
        vector3d.y = vector3d.y * invLength;
        vector3d.z = vector3d.z * invLength;
    }

    private static void unproject(Matrix4f mul, float winX, float winY, float winZ, int[] viewport, Vector3f dest) {
        Matrix4f matrix = mul.copy();
        matrix.invert();
        ...
        float invW = 1.0f / (floatBuffer.get(floatBuffer.position() + 0 * 4 + 3) * ndcX + floatBuffer.get(floatBuffer.position() + 1 * 4 + 3) * ndcY + floatBuffer.get(floatBuffer.position() + 2 * 4 + 3) * ndcZ + floatBuffer.get(floatBuffer.position() + 3 * 4 + 3));
        dest.set((floatBuffer.get(floatBuffer.position() + 0 * 4 + 0) * ndcX + floatBuffer.get(floatBuffer.position() + 1 * 4 + 0) * ndcY + floatBuffer.get(floatBuffer.position() + 2 * 4 + 0) * ndcZ + floatBuffer.get(floatBuffer.position() + 3 * 4 + 0)) * invW,
                (floatBuffer.get(floatBuffer.position() + 0 * 4 + 1) * ndcX + floatBuffer.get(floatBuffer.position() + 1 * 4 + 1) * ndcY + floatBuffer.get(floatBuffer.position() + 2 * 4 + 1) * ndcZ + floatBuffer.get(floatBuffer.position() + 3 * 4 + 1)) * invW,
                (floatBuffer.get(floatBuffer.position() + 0 * 4 + 2) * ndcX + floatBuffer.get(floatBuffer.position() + 1 * 4 + 2) * ndcY + floatBuffer.get(floatBuffer.position() + 1 * 4 + 2) * ndcZ + floatBuffer.get(floatBuffer.position() + 3 * 4 + 2)) * invW);
    }
}
